...
import { videos, videosWithExercises } from "./videos";
...
export interface CourseSummary {
  ...
  concepts: Counted[];
  /** How many lectures this course knows about (list + exercise-declared). */
  lectures: number;
}
...
export const courseSummaries: CourseSummary[] = courses
  .map((course) => {
    ...
    const ids = new Set([...course.lectures.map((l) => l.id), ...exs.map((e) => e.meta.video_id).filter(Boolean)]);
    return {
      course,
      exercises: exs,
      contributors: tallyAuthors(exs),
      concepts: tallyConcepts(exs),
      lectures: ids.size,
    };
  })
...
export const catalogStats = {
  ...
  practisedLectures: videosWithExercises.length,
